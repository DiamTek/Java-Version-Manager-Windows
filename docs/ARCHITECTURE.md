...
- **WiX Toolset v4 (MSI):** Automated build script (`packages\msi\build-msi.ps1`) that compiles native, per-user Windows Installers (`.msi`) bundling `jvm.bat`, companion branding assets, `uninstall.ps1`, `LICENSE`, and `README.md`:
  ...
  - **Deferred Action Execution Order**:
    ...
    - **Uninstall Hook**: Scheduled `Before="RemoveFiles"` to ensure PowerShell profile cleanup, Windows Terminal configuration scrubbing, and environment variable removal occur while the installed components are still present on disk.
  ...
  - **Automated Verification Suite (`packages\msi\test-msi.ps1`)**: Implements an automated 14-point synthetic integration suite designed for CI/CD pipelines and GitHub Actions build provenance attestations (`actions/attest-build-provenance`). The suite validates quiet installation (`msiexec /qn`), binary layout, registry metadata, PATH propagation, Windows Terminal profile injection, CLI help execution, quiet uninstallation, and total filesystem residual hygiene. Can be invoked directly from any working directory with automatic package discovery and compilation fallbacks.