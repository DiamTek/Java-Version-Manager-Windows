...
| **Suite 3** | **Reparse Point & Junction Non-Destructive Lifecycle** | **5** | Junction unbinding canary protection (verifying target JDK and canary files are not deleted when junctions are unbound), bracket-safe reparse querying with `-LiteralPath` and `$env:QUERY_PATH`, active dev repository `.git` guards, `jvm pin` refusal to overwrite symlink/junction `.java-version` targets, and `jvm clean` / `jvm prune` bottom-up junction unbinding in `downloads`/`temp`. |
| **Suite 4** | **Package Manager Manifest Integrity & Dry-Run** | **15** | Cross-package version parity (validating version sync across `jvm.bat`, Scoop, Chocolatey, and Winget), nuspec XML schema & UTF-8 No BOM validation, Chocolatey fail-closed SHA-256 checksum assertion, Scoop manifest schema, Winget multi-manifest coherence, `bump-version.ps1 -DryRun` working tree immutability, GitHub Actions 40-char SHA pinning & permissions audit, `jvm.bat` UTF-8 No BOM / 100% CRLF / EOF sentinel verification, `build-msi.ps1` RFC 4122 UUID v5 `Get-DeterministicGuid` collision resistance, `$profileCode` extraction & `WIX1103` script hash verification, TLS 1.2+1.3 protocol enforcement, non-HTTPS URI scheme (`http://`, `file://`, UNC) rejection, `:CheckUpdateStatus` & `:SelfUpdate` version/build downgrade attack rejection, SHA-256 mismatch fail-closed rejection, and malformed/spoofed `SHA256SUMS.txt` manifest rejection. |
| **Suite 5** | **Concurrency & Reparse Point Resilience** | **6** | Broken/dangling junction auto-recovery without deadlock, rapid sequential junction switching without lock corruption, parallel temp script isolation without startup interference, live ACL verification on `%JVM_SECURE_TEMP%` (`AreAccessRulesProtected`, no `Everyone`/`Users`), reparse-point junction rejection on `%JVM_SECURE_TEMP%` (`CWE-276`), and complete failure-path cleanup of `$zip.Dispose()` handles and partial extraction directories (`CWE-459`). |
| **Suite 6** | **Corrupt Registry Recovery & PATH Resilience** | **7** | `REG_SZ` vs `REG_EXPAND_SZ` type enforcement, non-blocking `SendMessageTimeout` broadcast execution under timeout constraints, `Set-JvmVar` storage boundary enforcement, `Set-JvmVar` CWE-22 sibling prefix collision (`.jdks_evil`) & strict variable allowlist (`COMSPEC` rejection), PowerShell AST parser validation of `:InstallPowerShellHook` emitted profile script, security parity between `install.ps1` and `jvm.bat`, and Base64 UTF-16LE `-EncodedCommand` privilege escalation boundary AST injection immunity (`CWE-250`). |
| **Suite 7** | **Uninstallation Safety & Marker Verification** | **6** | `Remove-DirectorySafely` target canary retention, refusal of uninstaller execution on directories lacking JVM markers, protected system root deletion blocking, strict ZipSlip boundary enforcement on extracted candidate directories, ZipSlip CWE-22 sibling-prefix collision (`../cand_evil/payload.exe`) & rooted entry (`\rooted_escape.exe`) rejection, and live malicious ZIP archive extraction testing across `../`, sibling-prefix, rooted `\`, and NTFS ADS `:` entries. |
