...
#### 2. What the 116 Automated Tests & 27 CWEs Cover
The test suite executes **116 automated test cases across 8 defensive suites** and outputs both per-test `[CWE-XX]` badges and a numerically sorted **27-CWE Coverage Summary** table (`CWE-20`, `CWE-22`, `CWE-41`, `CWE-59`, `CWE-66`, `CWE-73`, `CWE-78`, `CWE-88`, `CWE-155`, `CWE-250`, `CWE-276`, `CWE-295`, `CWE-319`, `CWE-330`, `CWE-345`, `CWE-354`, `CWE-377`, `CWE-400`, `CWE-426`, `CWE-427`, `CWE-428`, `CWE-459`, `CWE-494`, `CWE-532`, `CWE-601`, `CWE-611`, `CWE-918`), achieving a 100% pass rate:

| Suite | Focus Area | Tests | Key Adversarial & Security Vectors Verified |
|:---|:---|:---:|:---|
| **Suite 1** | **Adversarial Inputs & Fuzzing Defense** | **55** | Path traversal (`..`, `/`, `\`), single-dot aliases (`.`), reserved keyword `current`, command injection in `.java-version`/`.sdkmanrc`, branch traversal in `install.ps1`, profile hook regex injection filters, Win32 trailing dot/space bypasses (`current.`, `current `), legacy & extended DOS device names (`CON`, `PRN`, `AUX`, `NUL`, `NUL.jdk`, `CON.txt`) and Win32 raw device namespaces (`\\.\`), poison characters (`&`, `|`, `<`, `>`, `^`, `%`, `!`), `:RejectExclamationArg` across `%~1`..`%~9`, embedded double-quote smuggling (`_VSI_DQ`), `--vendor` flag poisoning, `jvm channel` / `jvm open` allowlisting, static `for /f` quote-stripping code audit, Alternate Data Streams (`:`), candidate traversal in `which`, leading hyphens (`--evil-flag`), multiple trailing dots/spaces, semicolon chaining, wildcards (`*`, `?`), `%SYS32%` CWD Trojan binary planting (`where.exe $PATH:java` / `findstr.exe`), prepended rogue `PATH` hijacking immunity, `jvm doctor` metacharacter resilience, `jvm pin` injection, `jvm exec` traversal/argument injection, `.java-version` UTF-8 BOM/CRLF/`jdk-` prefix resilience, `.sdkmanrc` read-only subcommand isolation (`--version` / `list`), `JVM_CALLER_PID` validation in `:EmitSessionEnv`, post-`latest` `LATEST_VER` validation in `:InstallCandidate` / `:SwitchCandidate`, `DL_STRIP_ROOT` leading-hyphen rejection, `:FetchAndExtract` `GetFullPath` canonicalization, and pipe-free `:SwitchCandidate` session `PATH` updates. |
...
| **Suite 3** | **Reparse Point & Junction Non-Destructive Lifecycle** | **7** | Junction unbinding canary protection (verifying target JDK and canary files are not deleted when junctions are unbound), bracket-safe reparse querying with `-LiteralPath` and `$env:QUERY_PATH`, active dev repository `.git` guards, `jvm pin` refusal to overwrite symlink/junction `.java-version` targets, `jvm clean` / `jvm prune` bottom-up junction unbinding in `downloads`/`temp`, `:InstallPowerShellHook` / `:RemovePowerShellHook` `ReparsePoint` symlink guard & `-LiteralPath` on `$PROFILE`, and `install.ps1` / `uninstall.ps1` `Test-HasReparsePointInLineage` guards across `$PROFILE`, `settings.json`, and Start Menu. |
| **Suite 4** | **Package Manager Manifest Integrity & Dry-Run** | **21** | Cross-package version parity (validating version sync across `jvm.bat`, Scoop, Chocolatey, and Winget), nuspec XML schema & UTF-8 No BOM validation, Chocolatey fail-closed SHA-256 checksum assertion & runtime HTTPS URI scheme validation (`CWE-319`), Scoop manifest schema, Winget multi-manifest coherence, `bump-version.ps1 -DryRun` working tree immutability, `bump-version.ps1` & `build-choco.ps1` SemVer/traversal validation (`CWE-20`), XML External Entity (XXE) & DTD prohibition (`DtdProcessing::Prohibit` — `CWE-611`), GitHub Actions 40-char SHA pinning & permissions audit, `jvm.bat` UTF-8 No BOM / 100% CRLF / EOF sentinel verification, `build-msi.ps1` RFC 4122 UUID v5 `Get-DeterministicGuid` collision resistance, `$profileCode` extraction & `WIX1103` script hash verification, TLS 1.2+1.3 protocol enforcement, non-HTTPS URI scheme (`http://`, `file://`, UNC) rejection, SSRF & vendor host allowlisting (`Test-TrustedJvmUri` — `CWE-918`), open redirect host verification (`CWE-601`), `:CheckUpdateStatus` & `:SelfUpdate` version/build downgrade attack rejection, SHA-256 mismatch fail-closed rejection, malformed/spoofed `SHA256SUMS.txt` manifest rejection, and `Nightly` channel Git Blob SHA-1 cryptographic verification (`blob <len>\0`), dynamic `%ProgramFiles%` (`%JVM_PF%`), & ARM64 resolver guards (`CWE-494`). |
| **Suite 5** | **Concurrency & Reparse Point Resilience** | **7** | Broken/dangling junction auto-recovery without deadlock, rapid sequential junction switching without lock corruption, parallel temp script isolation without startup interference, zero predictable `!RANDOM!` temp filenames (`[System.IO.Path]::GetRandomFileName` enforcement — `CWE-330`), live ACL verification on `%JVM_SECURE_TEMP%` (`AreAccessRulesProtected`, no `Everyone`/`Users`), reparse-point junction rejection on `%JVM_SECURE_TEMP%` (`CWE-276`), and complete failure-path cleanup of `$zip.Dispose()` handles and partial extraction directories (`CWE-459`). |
| **Suite 6** | **Corrupt Registry Recovery & PATH Resilience** | **9** | `REG_SZ` vs `REG_EXPAND_SZ` type enforcement, non-blocking `SendMessageTimeout` broadcast execution under timeout constraints, `Set-JvmVar` storage boundary enforcement, `Set-JvmVar` CWE-22 sibling prefix collision (`.jdks_evil`) & strict variable allowlist (`COMSPEC` rejection), PowerShell AST parser validation of `:InstallPowerShellHook` emitted profile script, security parity between `install.ps1` and `jvm.bat`, Base64 UTF-16LE `-EncodedCommand` privilege escalation boundary AST injection immunity (`CWE-250`), `:BackupRegistry` `fsutil` reparse-point query & `icacls /inheritance:r` DACL isolation on `%LOCALAPPDATA%\DiamTek\JVM\backups` (`CWE-532`), and `:InstallGlobalCommand` canonical `%LOCALAPPDATA%\DiamTek\JVM\bin` ACL & `DoNotExpandEnvironmentNames` protection (`CWE-427`). |
| **Suite 7** | **Uninstallation Safety & Marker Verification** | **7** | `Remove-DirectorySafely` target canary retention, refusal of uninstaller execution on directories lacking JVM markers, protected system root deletion blocking, strict ZipSlip boundary enforcement on extracted candidate directories, ZipSlip CWE-22 sibling-prefix collision (`../cand_evil/payload.exe`) & rooted entry (`\rooted_escape.exe`) rejection, live malicious ZIP archive extraction testing across `../`, sibling-prefix, rooted `\`, and NTFS ADS `:` entries, and `install.ps1` quoted `System32` `UninstallString` / `QuietUninstallString` (`-Quiet`) & pinned Start Menu shortcut `TargetPath` (`CWE-428`). |
