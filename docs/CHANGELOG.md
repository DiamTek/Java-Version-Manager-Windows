...
- **Status & Diagnostic Dashboard (`jvm current` / `jvm status`)**: Built a real-time diagnostic dashboard querying active Java vendor, version, `JAVA_HOME`, `java.exe` binary path, architecture switching mode (`[Symlink Mode]` vs `[Registry Mode]`), junction link target, and active JVM ecosystem candidates (`maven`, `gradle`, `kotlin`, etc.).
...
- **PowerShell Profile Hook Management (`jvm hook`)**: Added dedicated CLI commands (`jvm hook`, `jvm hook status`, `jvm hook remove`) and decoupled the PowerShell profile auto-sync hook from the global User PATH configuration in the Settings Menu, providing separate `[INSTALLED]` / `[NOT INSTALLED]` indicators for both.
