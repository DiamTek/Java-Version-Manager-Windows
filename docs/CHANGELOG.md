...
This release delivers an exhaustive defensive security hardening overhaul across the entire JVM runtime engine, UAC elevation boundaries, and package manifests, accompanied by a comprehensive 55-test automated adversarial security test suite.

### Security & Defensive Hardening
- **Complete Adversarial Input Sanitization (`:ValidateStrictIdentifier`)**: Engineered a centralized, multi-stage sanitization subroutine (`:ValidateStrictIdentifier`) in `jvm.bat` that validates all external inputs across `link`, `unlink`, `which`, candidate version switches, candidate installation, uninstallation, and configuration parsing:
  ...
  - *Poison Character & Metacharacter Neutralization*: Neutralizes dangerous `cmd.exe` command chaining and poison characters (`&`, `|`, `<`, `>`, `^`, `;`, `"`, `*`, `?`) via parameter substitution and regex validation, accompanied by two-pass `disabledelayedexpansion` checks against delayed expansion corruption from exclamation marks (`!`) and percent signs (`%`).
...
- **Process Elevation Hardening (Base64 UTF-16LE & System32 SpecialFolder Isolation)**: Overhauled administrative UAC elevation routines (`Start-Process powershell -Verb RunAs`) across `jvm.bat`, `install.ps1`, `uninstall.ps1`, and `build-msi.ps1`:
  ...
  - *Pure-Batch Call-by-Name Parameter Passing*: Refactored internal CMD subroutine dispatching in `jvm.bat` to pass argument variables strictly by name (`call :ValidateStrictIdentifier VAR_NAME`) rather than by value, completely neutralizing CMD's double-parsing pass on poison characters (`&`, `|`, `<`, `>`).
...
- **Package Manager Supply Chain Hardening**:
  - *Chocolatey MSI Distribution*: Migrated Chocolatey package scripts (`packages/choco/tools/chocolateyInstall.ps1` and `chocolateyUninstall.ps1`) from dynamic remote script execution (`install.ps1`) to static, pre-compiled Windows Installer (`.msi`) distribution using `Install-ChocolateyPackage` with cryptographic SHA-256 integrity verification (`checksumType64: 'sha256'`).
...
- **55-Test Automated Security & Adversarial Fuzzing Suite (`tests/Test-JvmSecurity.ps1`)**: Engineered an exhaustive, standalone 55-test automated security test suite covering 8 defensive suites with a 100% pass rate:
  1. *Adversarial Inputs & Fuzzing Defense (30 tests)*: Path traversal (`..`, `/`, `\`), single dot (`.`), `current` keyword guard, command injection in `.java-version`/`.sdkmanrc`, branch traversal in `install.ps1`, profile hook injection filtering, Win32 trailing dot/space bypasses (`current.`, `current `), DOS reserved devices (`CON`, `PRN`, `AUX`, `NUL`), poison characters (`&`, `|`, `<`, `>`, `^`, `%`, `!`), ADS streams (`:`), candidate traversal in `which`, leading hyphens (`--evil-flag`), multiple trailing dots/spaces, semicolon chaining, and wildcards (`*`, `?`).
  ...
  4. *Package Manager Manifest Schema Integrity & Dry-Run Guarantees (5 tests)*: Cross-package version synchronization across Scoop, Chocolatey, Winget, and `jvm.bat`; nuspec XML validation and UTF-8 No BOM checks; Scoop manifest schema; Winget multi-manifest coherence; and `bump-version.ps1 -DryRun` working tree immutability.
  5. *Concurrency & Reparse Point Non-Destructive Resilience (2 tests)*: Broken/dangling junction auto-recovery without deadlock and rapid sequential junction switching.
  ...
  7. *Uninstallation Safety & Marker Verification (3 tests)*: `Remove-DirectorySafely` target canary retention, refusal of uninstaller execution on directories lacking JVM markers, and system root deletion blocking.
