...
This release delivers an exhaustive defensive security hardening overhaul across the entire JVM runtime engine, UAC elevation boundaries, CI/CD pipelines, and package manifests, accompanied by a comprehensive 62-test automated adversarial security test suite.
...
- **Complete Adversarial Input Sanitization (`:ValidateStrictIdentifier` & `:RejectExclamationArg`)**: Engineered a centralized, multi-stage sanitization pipeline in `jvm.bat` that validates all external inputs across `link`, `unlink`, `which`, `pin`, `exec`, candidate version switches, candidate installation, uninstallation, and configuration parsing:
  ...
  - *Poison Character & Metacharacter Neutralization*: Neutralizes dangerous `cmd.exe` command chaining and poison characters (`&`, `|`, `<`, `>`, `^`, `;`, `"`, `*`, `?`) via pure-batch character loop inspection (`:VSI_CharLoop`) and internal substitution checks without spawning subshells or piping untrusted input.
...
- **System32 Binary Pinning & CWD Planting Immunity (`CWE-426` / `CWE-427`)**:
  ...
  - Hardened all 60+ `for /f ('%..._BIN% ...')` command loops to preserve inner argument quoting around `-Command "..."` and registry queries without `cmd.exe /c` outer-quote stripping.
...
- **Strict ZipSlip Archive Extraction Boundary Enforcement**:
  - Enforced canonical destination boundary checks (`$destinationPath.StartsWith($fullRoot, [System.StringComparison]::OrdinalIgnoreCase)`) during ZIP archive extraction in `:DownloadFile`, aborting immediately if any archive entry attempts to escape the target candidate directory.
...
- **Package Manager, MSI & CI/CD Supply Chain Hardening**:
  ...
  - *Deterministic MSI `ProductCode` & Per-User Scope*: Added deterministic UUID v5 `ProductCode` generation (`Get-DeterministicGuid`) in `packages/msi/build-msi.ps1`, configured dual-purpose per-user/per-machine installation scope (`ALLUSERS=2`, `MSIINSTALLPERUSER=1`), and added `DefaultVersion` file metadata.
...
- **62-Test Automated Security & Adversarial Fuzzing Suite (`tests/Test-JvmSecurity.ps1`)**: Engineered an exhaustive, standalone 62-test automated security test suite covering 8 defensive suites with a 100% pass rate:
  1. *Adversarial Inputs & Fuzzing Defense (34 tests)*: Path traversal (`..`, `/`, `\`), single dot (`.`), `current` keyword guard, command injection in `.java-version`/`.sdkmanrc`, branch traversal in `install.ps1`, profile hook injection filtering, Win32 trailing dot/space bypasses (`current.`, `current `), DOS reserved devices (`CON`, `PRN`, `AUX`, `NUL`), poison characters (`&`, `|`, `<`, `>`, `^`, `%`, `!`), ADS streams (`:`), candidate traversal in `which`, leading hyphens (`--evil-flag`), multiple trailing dots/spaces, semicolon chaining, wildcards (`*`, `?`), `%SYS32%` CWD binary planting defense, `jvm pin` metacharacter injection, `jvm exec` path traversal, and `jvm install` candidate version injection.
  2. *Registry & Environment Variable Boundary Tests (5 tests)*: User PATH `REG_EXPAND_SZ` preservation, Base64 UTF-16LE elevation payload integrity, elevation path resolution immunity to SystemRoot environment saturation, Oracle `javapath` de-bloating, and extreme PATH lengths (>2048 characters).
  ...
  4. *Package Manager Manifest Schema Integrity & Dry-Run Guarantees (6 tests)*: Cross-package version synchronization across Scoop, Chocolatey, Winget, and `jvm.bat`; nuspec XML validation and UTF-8 No BOM checks; Chocolatey fail-closed SHA-256 checksum assertion; Scoop manifest schema; Winget multi-manifest coherence; and `bump-version.ps1 -DryRun` working tree immutability.
  5. *Concurrency & Reparse Point Non-Destructive Resilience (3 tests)*: Broken/dangling junction auto-recovery without deadlock, rapid sequential junction switching, and parallel temp script isolation without startup interference.
  6. *Corrupt Registry Recovery & PATH Resilience (3 tests)*: `REG_SZ` vs `REG_EXPAND_SZ` type enforcement, non-blocking `SendMessageTimeout` execution, and `Set-JvmVar` storage boundary enforcement.
  7. *Uninstallation Safety & Marker Verification (4 tests)*: `Remove-DirectorySafely` target canary retention, refusal of uninstaller execution on directories lacking JVM markers, system root deletion blocking, and strict ZipSlip boundary enforcement on extracted candidate directories.
