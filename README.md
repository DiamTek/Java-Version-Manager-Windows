...
* **Directory-based Auto-Switching (.java-version & .sdkmanrc):** Instantly switch to a project's required JDK version by simply running `jvm` inside any directory containing a `.java-version` or SDKMAN `.sdkmanrc` file. The tool parses the file and seamlessly swaps your environment in the background. Unlike SDKMAN which only stores static versions, our `.java-version` engine is incredibly advanced: it natively supports parsing full CLI flags directly from the file (e.g., `21 --vendor adoptium --legacy`), allowing you to lock specific vendors or architecture modes on a strict per-project basis. And if you're collaborating with SDKMAN users on macOS/Linux, JVM will happily hijack their `.sdkmanrc` files on Windows and map their vendor strings (`-tem`, `-amzn`, etc.) directly to your native JDKs.
...
* **Built-in Self-Updater with Integrity Verification:** Run `jvm version` (or `-v`) to trigger the intelligent semantic versioning engine. It securely mathematically compares your local build against the remote GitHub `main` branch (e.g. `20260830.10` > `20260830.2`). If an update is available, `jvm self-update` will verify the downloaded file contains a valid `:: END OF SCRIPT` sentinel before atomic-swapping the core script, protecting against truncated or corrupted downloads.
