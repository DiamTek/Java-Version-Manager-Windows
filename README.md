...
* **v1.0.2:** Comprehensive defensive security hardening, elevation boundary immunization, and adversarial fuzzing suite. Replaced user-writable `$env:SystemRoot` and `%SystemRoot%` lookups during process elevation with native immutable Win32 SpecialFolder APIs (`[Environment]::GetFolderPath([Environment+SpecialFolder]::System)`), immunizing elevation boundaries against user-level environment variable saturation/spoofing (CWE-426). Pinned filesystem root safeguards in `uninstall.ps1` to immutable `SpecialFolder` locations (`Windows`, `UserProfile`, `ProgramFiles`, `ProgramFilesX86`). Eliminated CMD quote-toggling command injection on line 84 in `jvm.bat` by removing unused `set "ORIGINAL_ARGS=%*"`. Engineered pure-batch character loop validation (`:ValidateStrictIdentifier` / `:VSI_CharLoop`) and call-by-name parameter passing (`call :Sub VAR_NAME`), neutralizing CMD second-pass tokenization and argument splitting on poison characters (`&`, `|`, `<`, `>`, `^`). Routed `link` and `unlink` CLI invocations via direct `goto :HANDLE_LINKS` to bypass CMD argument splitting on raw CLI parameters. Preserved `REG_EXPAND_SZ` registry kind across User and Machine PATH with combined length boundaries (2,048-char warning / 8,191-char abort limit) and non-blocking `SendMessageTimeout` setting broadcasts (5,000ms with `SMTO_ABORTIFHUNG`). Re-architected directory junction cleanup in `uninstall.ps1` and `build-msi.ps1` using `Remove-DirectorySafely` to sort reparse points bottom-up and unbind junctions without traversing into target JDK installations. Engineered an exhaustive 55-test automated adversarial security test suite (`tests/Test-JvmSecurity.ps1`) covering 8 defensive suites with a 100% pass rate.
