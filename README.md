...
* **v1.0.2:** Comprehensive defensive security hardening, elevation boundary immunization, supply chain pinning, and 78-test adversarial fuzzing suite. Pinned all external Windows system binaries (`reg.exe`, `findstr.exe`, `where.exe`, `choice.exe`, `timeout.exe`, `fsutil.exe`, `powershell.exe`, `chcp.com`, `icacls.exe`, `explorer.exe`) to `%SYS32%` (`%..._BIN%`) and enforced `NoDefaultCurrentDirectoryInExePath=1` to eliminate CWD binary planting and `PATH` search-order hijacking (CWE-426 / CWE-427). Added `:RejectExclamationArg` under `setlocal disabledelayedexpansion` prior to `setlocal enabledelayedexpansion` so `cmd.exe` cannot silently strip unpaired `!` poison characters (`foo!bar` -> `foobar`). Isolated temporary helper scripts and `.jvm_session_target` inside an ACL-locked (`icacls /inheritance:r`) per-user temp directory (`%LOCALAPPDATA%\DiamTek\JVM\temp`) with cryptographically random filenames (CWE-377 / CWE-378). Enforced strict ZipSlip canonical boundary validation (`$destinationPath.StartsWith($fullRoot, [System.StringComparison]::OrdinalIgnoreCase)`) with trailing directory separators (`\`) and explicit rooted-entry rejection during archive extraction, and hardened `Set-JvmVar` `$allowedRoots` against sibling-prefix collisions (CWE-22). Replaced user-writable `$env:SystemRoot` and `%SystemRoot%` lookups during process elevation with native immutable Win32 SpecialFolder APIs (`[Environment]::GetFolderPath([Environment+SpecialFolder]::System)`), immunizing elevation boundaries against user-level environment variable saturation/spoofing. Pinned filesystem root safeguards in `uninstall.ps1` to immutable `SpecialFolder` locations (`Windows`, `UserProfile`, `ProgramFiles`, `ProgramFilesX86`). Engineered pure-batch character loop validation (`:ValidateStrictIdentifier` / `:VSI_CharLoop` with `_VSI_DQ` quote-smuggling detection) neutralizing CMD second-pass tokenization on poison characters (`&`, `|`, `<`, `>`, `^`, `;`, `"`, `*`, `?`, `!`, `%`). Preserved `REG_EXPAND_SZ` registry kind across User and Machine PATH with combined length boundaries (2,048-char warning / 8,191-char abort limit) and non-blocking `SendMessageTimeout` setting broadcasts (5,000ms with `SMTO_ABORTIFHUNG`). Re-architected directory junction cleanup in `uninstall.ps1` and `build-msi.ps1` using `Remove-DirectorySafely` to sort reparse points bottom-up and unbind junctions without traversing into target JDK installations. Pinned all GitHub Actions in `ci.yml` and `release.yml` to verified 40-character commit SHAs, added deterministic UUID v5 `ProductCode` generation in `build-msi.ps1`, enforced fail-closed SHA-256 checksum assertions in `chocolateyInstall.ps1`, and engineered an exhaustive 78-test automated adversarial security test suite (`tests/Test-JvmSecurity.ps1` with `-Suite`/`-Filter`, per-suite timing telemetry, and `$env:GITHUB_STEP_SUMMARY` Markdown export) covering 8 defensive suites with a 100% pass rate.
